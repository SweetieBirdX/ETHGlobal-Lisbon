...
- **The policy floor and the track quote are two separate knobs, and the floor is set below the catalogue on purpose.** The floor comes from the rights holder's sentence (`minPricePerShareHbar × shares`); the price charged is the track's own rate (`quotePrice`). The seeded catalogue runs **0.00082–0.00198 ℏ per share** and the shipped default floor is **0.0008**, which sits under the cheapest track — so an offer at *any* track's own asking price clears the policy, and the floor does what a floor should: refuse lowballs, not price the catalogue. On a 5% licence on the cheapest track that is a 0.4 ℏ floor against a 0.41 ℏ quote. If you reseed with different prices, keep the floor at or below the cheapest track, or offers at that track's own rate will be refused.
